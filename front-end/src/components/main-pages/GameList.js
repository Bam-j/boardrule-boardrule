                ...
                <AutoSizer disableHeight>
                    {({width}) => (
                        <StyledGameList className={'GameList'} autoHeight height={height} width={width} isScrolling={isScrolling}
                                        overscanRowCount={0} onScroll={onChildScroll} scrollTop={scrollTop}
                                        rowCount={games.length} rowHeight={150} rowRenderer={rowRenderer} list={games}
                                        deferredMeasurementCache={cache} style={{outline: 'none'}}/>
                    )}
                </AutoSizer>
            )}
        </WindowScroller>
